...
colnames(auc_df) <- c('low_CI', 'AUC', 'high_CI')
rownames(auc_df) <- c('Both', 'Only_I', 'Only_II')
write.table(tabgene, file = paste("/cellar/users/ramarty/Data/hla_ii/generated_data/predictions.", args[1], ".txt", sep=''))

# Plot the ROCs
pdf(paste('/cellar/users/ramarty/Data/hla_ii/generated_figures/predictions/ROC.threshold_', args[1], '.pdf', sep=''))
